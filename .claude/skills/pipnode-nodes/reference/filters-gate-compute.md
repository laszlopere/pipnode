# Filters — Gate, Expressions, Compute & AI, Vectors

Filters are `PnNode` subclasses that sit *inline* on a wire: they all set both `has_input` and `has_output`, so a message arrives, the node transforms or gates it, and (usually) one message goes out the other side. Three sub-categories appear under the palette's **Filters** group: **Filters/Gate** nodes decide pass/block (and some rewrite `data.value` to a clean 0.0/1.0 boolean), **Filters/Expressions** nodes recompute the bag (Calculator, Calculator 2, Calculator Engine, JMESPath, Parse JSON), **Filters/Compute & AI** nodes derive new values (FX Converter, Fiat Converter, Throughput), and **Filters/Vectors** nodes produce and collapse `$pnvector` buffers (Ramp, Reduce). Every node forwards the *received* topic unchanged unless noted (Throughput and Ramp are the exceptions — each mints a brand-new message, which therefore carries the *node's own* resolved topic). Each output wire receives a deep copy. The message contract: envelope `topic`/`id`/`created` plus a schemaless `data.*` bag whose mandatory members are `data.value` (canonical number; boolean = 0.0/1.0, test `> 0.5`), `data.output` (human summary) and `data.success` (boolean).
...
**Behaviour** — Binds every numeric (int64/double) member of `data` as a variable under its name (`data.value`→`value`, `data.temp`→`temp`) — the **FLAT** rule, which lives in `pn_expr_bind_flat` (`lib/pn-expr-bind.c:65`) so Calculator 2 and the Figure node share one copy. A `$pnvector` member binds as a **vector** variable, so the expression can broadcast/elementwise over it and the result can itself be a vector (`lib/pn-expression.c:129`). Language: numbers, `+ - * /`, parens, unary minus; `%` floored modulo (sign of the divisor: `-1 % 8` = 7; `x % 0` = NaN); bitwise `& | ^ << >>` and unary `~` on the operand truncated to int64 (`^` is XOR, **not** power; `>>` is arithmetic; a non-finite / out-of-int64 operand or a shift count outside 0..63 yields NaN, not an error); comparisons `< > <= >= == !=` (loosest binding, yield 1.0/0.0). Precedence tightest first: unary, `* / %`, `+ -`, `<< >>`, `&`, `^`, `|`, comparisons — Python's order, so `value & 1 == 1` is `(value & 1) == 1`; one-arg functions `sin cos tan asin acos atan cot sec csc degrees radians sinh cosh tanh asinh acosh atanh log ln log10 log2 log1p exp exp2 expm1 sqrt cbrt abs frac sign factorial isnan isinf isfinite sinc erf erfc j0 j1` (`frac(x)` = x - floor(x), **always in [0, 1)** — `frac(-0.25)` = 0.75; `sign` is -1/0/1 with `sign(0)` = 0; an out-of-domain argument such as `asin(2)` yields NaN); two-arg `atan2(y, x)`, `min`, `max` (fmin/fmax — a NaN operand is skipped), `pow(x, y)` (the only power: `^` is XOR), `hypot(x, y)`, `fmod(x, y)` (truncated remainder, keeps x's sign, unlike the floored `%`), `copysign(x, y)`, `gcd(a, b)`, `lcm(a, b)`, `step(edge, x)`, `pct(x, p)` (x*p/100), `pct_change(old, new)` ((new-old)/old — **old first**) and `bps(x, b)` (x*b/10000); three-arg `clamp(x, lo, hi)` (crossed bounds return `lo`, a NaN value stays NaN), `if(cond, a, b)` (a **true select** — the arm not chosen cannot poison the result; a *comparison* condition collapses a vector to one 1.0/0.0, a *function* condition maps per element), `lerp(a, b, t)`, `smoothstep(lo, hi, x)`, `compound(principal, rate, periods)`, `fv(pmt, rate, nper)`, `pv(pmt, rate, nper)`, `pmt(pv, rate, nper)` (rate **per period**; r = 0 is a limit, not a division); **two rounding rules, both named**: `round` is half-**away**-from-zero (`round(2.5)`=3) and `rint` is ties-to-**even** (`rint(2.5)`=2, banker's); the five verbs `floor ceil round trunc rint` each take an **optional second argument** = decimal places (`round(x, 2)` to the cent, `round(1234, -2)`=1200; these QUANTISE a value, they are not display formatting — that is the Numeric node's decimal-places); and `log(x[, base])`, whose arity is a range (comma-separated — a call with the wrong argument count is a **parse** error, reported as you type, naming what the function takes down to "1 or 2 arguments"). An undefined result is a **value, not an error**: `sqrt(-1)`/`acosh(0)` are nan, `log(0)`/`cot(0)` are ±inf, and they travel on down the wire — the **one exception** is `factorial`/`gcd`/`lcm`, the only rows with an argument check: a fraction, a negative, a nan, an infinity or (for `factorial`) anything above 170 fails evaluation with `PN_VAR_STORE_ERROR_BAD_ARGUMENT` ("factorial: takes a whole number"); there is deliberately **no `time()`** — use the Clock node's `value`; constants `pi` and `e`, which resolve only when nothing is bound under that name, so a `data.pi` member shadows the constant. Multiple newline-separated statements evaluate in order; `name = expr` binds `name` for later lines; `data.value` = the last statement's value.

**Writes** — Success: `data.value` = result, `data.success` = true, `data.output` = formatted result — `%g` for a scalar, but a **vector** result renders as `"[a, b, …] (N values)"` via the shared bounded renderer `pn_var_store_value_to_string` (`lib/pn-var-store.c:426`) and is written back with `pn_message_set_vector` rather than `set_double` (`lib/pn-expression.c:138`); every assigned name added as a numeric member. Failure (parse or eval error, e.g. unknown variable/function): `data.success` = false, `data.error` + `data.output` = reason, `data.value` **left unchanged**. Message forwarded either way (never dropped).
...
**Writes** — Identical to Calculator, vectors included (a vector result goes back through `pn_message_set_vector`, `lib/pn-expression2.c:154`): success sets `data.value`/`data.success`/`data.output` + assigned names; failure sets `success`=false + `data.error`/`data.output`, leaves `data.value`. Forwarded either way. Note: a referenced input that hasn't fired yet (e.g. `value2` still unknown) → eval failure, flagged not dropped.

**Gotchas** — `is_input_name()` (`lib/pn-expr-bind.c:96`, reached through the **COLLATED** rule `pn_expr_bind_collated`, `lib/pn-expression2.c:141`) distinguishes core-injected per-input values (bound by bare name) from this message's siblings (suffixed). The bare reserved `value` member is skipped in the sibling pass. Re-open the dialog to refresh the Input-name fields after changing `inputs`. Purple body, fa-calculator icon.
...
**Purpose** — Crypto/fiat exchange-rate converter: multiplies `data.value` by a cached `from→to` factor and forwards, stamping the factor and target code. (`lib/pn-rate.c:674` receive.) Subclass of **PnHttp** (→ PnAutoTrigger) — periodically fetches the rate itself.
...
**Writes** — Rewrites `data.value` (× rate), and always stamps `data.rate` (the factor), `data.currency` (the `to` nick, e.g. "USD") and `data.deprecated` (boolean — TRUE while no successful fetch has landed for the current pair, or the last one failed, so a downstream Debug/Format can flag the conversion as untrustworthy; the value still passes through converted so chains do not stall; `lib/pn-rate.c:712`). Does not set `data.success`/`data.output`. Notably does **not** emit on its own periodic ticks — rate updates are internal state; output is 1:1 with input messages.

**Gotchas** — There is also a `status` property (string, default `"Never updated"` = `PN_RATE_STATUS_NEVER`, `lib/pn-rate.c:946`): READWRITE so it serialises, but rendered read-only in the dialog — the same cache-status row the Fiat Converter has. `rate` defaults to 1.0, so an unconfigured/not-yet-fetched node passes values through scaled by 1. State is mutex-guarded (worker writes, main thread reads on receive). 10-minute floor protects CoinGecko's free-tier rate limit. GUI (icon+ticker combos, read-only cache rows) lives in companion `pn-rate-gui.c`. Gold body, fa-exchange icon.
...
**Writes** — Mints a **new** message via `pn_message_new(node, NULL)` (the original message and its topic are **not** forwarded; the NULL template means the new message carries the **node's own resolved topic** — `lib/pn-stats.c:264` + `lib/pn-message.c:328`). Fields: `data.count` (int64), `data.min_seconds`, `data.max_seconds`, `data.average_seconds`, `data.per_minute`, `data.per_hour`, `data.per_day` (all double). Does not carry `data.value`/`success`/`output` from input.

**Gotchas** — The only node here that doesn't forward the received topic — downstream sees `/pnode/Throughput/<nodename>`, the node's own default. There is **no** `stats` topic: `pn-stats.c` never mentions `topic` at all, and sets no topic template. Gaps larger than a single bin (even larger than the whole window) are still recorded as one measurement at the bin where the later arrival landed. Window/bin change drops the ring and the last-arrival latch (no migration). Yellow body, fa-bar-chart icon.

# Filters/Vectors

Two nodes that bracket the large-vector machinery (TODO #43): **Ramp** produces a `$pnvector` buffer out of three scalars, **Reduce** collapses one back to a scalar. Between them sits everything that already speaks vectors — the Calculator pair (a `$pnvector` member binds as a vector variable), Oscilloscope, Graph, Figure. Both nodes are GTK-free core nodes with no `-gui.c` companion and no settings beyond what is listed.

The vector itself travels **out of band**: `data.value` holds a `$pnvector` marker rather than a JSON array, and a consumer resolves it with `pn_message_resolve_vector()`. That is what keeps ten million doubles off the JSON path.

## Ramp

**Purpose** — Three-input vector generator: latches a number per input and, once all three have arrived, emits a fresh message whose `data.value` is a `$pnvector` of N evenly-spaced values from `from` to `to`. (`lib/pn-ramp.c`)

**When to use** — To make the x-axis of anything: a sweep for the Calculator to evaluate elementwise, a time base for Oscilloscope or Graph, a sample grid for a Figure plate. It is the producer half of the vector family; **Reduce** is its dual.

**Ports** — 3 **named** inputs (use `target_input`): 0 `from`, 1 `to`, 2 `N` (`pn-ramp.c:193`). One output.

**Settings** — **none at all.** The node installs no `g_param_spec_*` and has no get/set_property: everything is wired, nothing is persisted.

**Reads** — `data.value` on whichever input the message arrived at; must be a JSON int64 or double (`read_value`, `pn-ramp.c:59`). A non-numeric or missing value is ignored in silence and leaves the latch untouched. An input index outside 0–2 returns.

**Gating** — Emits nothing until all three `has_*` flags are set (`pn-ramp.c:134`). After that, **any** numeric message on **any** input recomputes from the three latched values and re-emits — so driving `N` alone changes the resolution, `to` alone stretches the span.

**Writes** — Mints a brand-new `pn_message_new (node, NULL)` (so it carries the node's own resolved topic) and sets **only** `data.value` as a vector (`pn_message_set_vector`, `pn-ramp.c:150`). No `success`, no `output`. The triggering message is **consumed, never forwarded**.

**The ramp itself** (`build_ramp`, `pn-ramp.c:79`) — `N` is rounded `floor(n + 0.5)`; `< 1` emits nothing; clamped to `PN_RAMP_MAX_N` = **10 000 000** (`pn-ramp.c:34`, ≈80 MB of doubles — generous, but it stops a stray `N` of a billion from eating the machine's RAM). `len == 1` yields the single sample `from`; otherwise `step = (to - from) / (len - 1)` and the **last element is pinned exactly to `to`** so rounding never leaves the ramp a hair short of its endpoint.

**Gotchas** — Nothing is saved, so a reloaded worksheet stays silent until all three inputs have fired again (there is no startup announce). A non-numeric message is *not* a reset. Teal body `{0.30, 0.70, 0.74}`, fa-sort-amount-asc U+F160.

## Reduce

**Purpose** — Vector → scalar fold: collapses a `$pnvector` on `data.value` to one number and forwards, re-entering the scalar world the rest of a flow lives in. (`lib/pn-reduce.c`)

**When to use** — Immediately downstream of anything vector-valued, to get a number a gauge, Threshold or Comparator can use: the mean of a sweep, the max of a trace, the count of samples.

**Ports** — one input, one output (`pn-reduce.c:272`).

**Settings** — `mode` — enum `PnReduceMode`, default **`mean`** (`pn-reduce.c:275`). Ten folds, whose nicks double as the combo labels and are **serialized by nick**: `sum mean min max range count product stddev first last` (`pn-reduce.c:56`). The numeric enum values are fixed so saved worksheets keep their selection.

**Reads** — `data.value` **only**. `pn_message_resolve_vector` turns a `$pnvector` marker into the buffer and the whole thing is folded; a plain int64/double folds as a **one-element** vector; anything else — string, boolean, object, missing — is **silently dropped with no emit** (`pn-reduce.c:172`).

**Fold semantics** (`fold`, `pn-reduce.c:86`) — An empty buffer yields `1.0` for `product` and `0.0` for every other mode. `range` = max − min. `stddev` is the **population** standard deviation (÷ n, not n−1). `first` = `d[0]`, `last` = `d[n-1]`. For one element: sum = mean = min = max = first = last = the value, count = 1, range = 0, stddev = 0.

**Writes** — Overwrites `data.value` with the scalar (`pn_message_set_double`, `pn-reduce.c:201`) and forwards the **same** message, leaving topic, id and every other member intact. Does not touch `success` or `output`.

**Gotchas** — Hard-wired to `data.value`: there is no source-field property, so a vector parked on another member is invisible to it. Booleans are **not** coerced to 1.0/0.0 here (unlike Value Router / Value Trend) — a boolean `value` is dropped silently. No `pn_node_set_has_error` path at all, so a wrongly-wired Reduce looks healthy and simply says nothing. The default is `mean`, not `sum`. Blue body `{0.30, 0.62, 0.74}`, fa-compress U+F066.
