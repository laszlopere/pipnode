...
Every node carries a `hostname` string property (default **`""`**, the empty string) wired through `pn_param_spec_set_hostname_hint()`, which makes the settings dialog render `g_get_host_name()` as a grey placeholder hint. `hostname_is_local()` maps NULL, `""`, **and** the literal `"localhost"` (case-insensitive) to the local path; any other value is treated as a passwordless-SSH target.

**Every node also carries an `auth-profile` property** — a profile-ref to an **SSH Login** credential, installed from the shared `pn_ssh_auth_profile_param_spec()` (`lib/pn-ssh-profile.c:295`), plus an `on_vault_changed` handler so an edited credential takes effect live. It is present on all eight nodes (e.g. `pn-load.c:425`, `pn-connections.c:487`).

**A blank `hostname` therefore no longer means "local" unconditionally.** `pn_ssh_login_effective_host()` (`lib/pn-ssh-profile.c:281`) lets an attached profile's own host win over the node's, because an SSH Login credential names the one host it may reach — so a node with an empty `hostname` but such a profile attached samples **remotely** (`pn-load.c:239`). The node's `hostname` is only the fallback for the credential-less case.

**The real ssh argv** is built by `pn_ssh_build_argv()` (`lib/pn-ssh-profile.c:324`), in this order: `ssh -o BatchMode=yes -o StrictHostKeyChecking=<policy> -o ConnectTimeout=<n>`, then `-l <user>`, `-p <port>` (only when set and ≠ 22) and `-i <identity>` (path expanded) when the profile supplies them, then the target host, then the base command. The host-key **policy comes from the profile**, not a hardcoded `accept-new` (that is merely the default when no profile is attached). BatchMode is unconditional — no interactive prompt is ever acceptable from a node running headless behind a desktop launcher, which is why a passphrase/password profile needs the separate TODO #47.5 path and secrets are deliberately not part of `PnSshLogin`. The configured-vs-real distinction is cosmetic on the wire: when local, `data.host` and the summary still report the **real machine name** (`g_get_host_name()`), never `"localhost"`, so a worksheet is self-describing (e.g. `pn-load.c:220-225`). The `hostname` string is mutex-guarded (worker thread reads, main thread writes); the delta-based nodes (CPU, Disk I/O, Network I/O) additionally reset their `have_prev` baseline under that lock on any host/device/interface change so a delta is never computed across two different machines/rows.
...
**Settings** —
- `hostname` (string, default `""`) — local `/proc/loadavg` read, or SSH target; grey local-name hint
- `auth-profile` (profile-ref to an SSH Login credential, default `""`) — see the shared section above; an attached profile's host **overrides** `hostname` (`pn-load.c:411-412`).
...
All seven share the System-Load shape above: `PnAutoTrigger` source, no input, violet body, `hostname` + `auth-profile` properties with the local/SSH pattern + grey hint, `host`/`success`/`output` always written, emit every tick. They differ in the metric, default `period`, extra settings, and the extra `data.*` members:
...
**Settings** — `hostname` + `auth-profile`; `core` (string, default `""` = aggregate `cpu` row → whole machine; `"0"`/`"1"`… → `cpu0`/`cpu1` for a single core, `pn-cpu.c:616`); inherited `period` (default 2).
...
**Settings** — `hostname` + `auth-profile`; `unit` (enum `PnMemoryUnit`: `%` default, or `B`/`KByte`/`MByte`/`GByte`, `pn-memory.c:637-650`); inherited `period` (default 2).
...
**Settings** — `hostname` + `auth-profile`; `aggregation` (enum `PnTempAggregation`: `average` default / `maximum`, `pn-temp.c:903-912`); inherited `period` (default 5).
...
**Settings** — `hostname` + `auth-profile`; `aggregation` (enum `PnAmbientAggregation`: `average` default / `maximum`); inherited `period` (default 5).
...
**Settings** — `hostname` + `auth-profile`; `device` (string, default `""`=all; e.g. `"sda"`, `"nvme0n1"`, `"dm-0"`); `unit` (enum `PnDiskIoUnit`, **default GByte/sec**, decimal 1 KByte=1000 B, `pn-disk-io.c:875-886`); inherited `period` (default 2).
...
**Settings** — `hostname` + `auth-profile`; `interface` (string, default `""`=all non-loopback; e.g. `"eth0"`, `"wlan0"`, `"enp86s0"`); `unit` (enum `PnNetIoUnit`, **default MByte/sec**, decimal, `pn-net-io.c:773-7848`); inherited `period` (default 2).
...
**Settings** — `hostname` + `auth-profile` (doc notes container coverage needs root, `pn-connections.c:475-481`); inherited `period` (default 5). No other settings.
