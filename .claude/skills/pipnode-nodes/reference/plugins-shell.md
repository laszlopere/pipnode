...
- `output-format` (enum `PnShellOutputFormat`, default **Text**) — Text = the historical contract (combined stdout+stderr → `data.output`); **JSON** = parse stdout and let the command define the payload (object merges into `data.*`, bare scalar → `data.value`, invalid JSON → `success=FALSE` carrying the raw text). Shared helper `pn_shell_output_apply` in `pn-shell-output.c`.
...
- `output-format` (enum `PnShellOutputFormat`, default **Text**) — same as Shell Command: Text → `data.output`; **JSON** → parse stdout (object merges into `data.*`, bare scalar → `data.value`, invalid JSON → `success=FALSE`). On the **Settings** tab. This is how a script feeds a numeric reading (e.g. `date +%s` → `data.value`) into a gauge/clock; see `examples/misc/clock_script.json`.
- inherited `period` (default **5 s**) / `autostart`.
- Dialog tabs: **Settings** (output-format, host, auth-profile) then **Script** (the full-width `sh` code editor alone, rightmost).
**Emits / consumes** — Timer-driven source (no input). Each tick spawns `/bin/sh -c <script>` locally (or hands the script to the remote shell over ssh). `${nodeclass}`/`${nodename}`/`${hostname}` are interpolated first via `pn_node_expand_vars`; other `${…}` is left for the shell. In Text mode writes **`data.success`** + **`data.output`** (no `data.value`); in JSON mode the parsed stdout shapes the bag. Logically identical to Shell Command — the only differences are cosmetic (own icon `fa-file-code-o` U+F1C9 / palette name) and the code-editor dialog.
