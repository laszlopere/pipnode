...
**Ports** — input only; `has_input = TRUE`, `has_output = FALSE` — terminal sink (`pn-sci-fi-sound.c:360-361`, `:403-404`). The inherited `topic` settings row is hidden (no output) (`:365-366`).
...
**Gotchas** —
- **Dispose stops the player.** Destroying the node cancels the in-flight playback and force-exits the subprocess (`pn_sci_fi_sound_dispose`, `pn-sci-fi-sound.c:298`, installed at `:346`), so deleting a node mid-clip does not leave it sounding.
