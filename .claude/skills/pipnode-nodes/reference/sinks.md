...
- Plates built on it so far: `examples/displays/pendulum.json` (TODO #85.1), `examples/displays/refraction.json` (#85.9), `examples/displays/bar-magnet.json` (#85.12) and the thin-lens proving sheet `examples/displays/ray-optics.json` (#80.15.2; the lever, 80.15.1, is still open). TODO #85 (twelve physics plates) is still open — the rest wait partly on 80.19, since there are **no arrows, angle-mark arcs, rotated labels or hatching yet** (the shipped plates hand-build arrowheads and needles out of `poly`).
