...
/* fa-cubes U+F1B3 — multiple-containers glyph in the FA-4 range the
 * bundled icon font supports.  Same FA-4 ceiling (no codepoints past
 * U+F2FF) the Free Command node's icon comment spells out applies
 * here. */
#define PN_LXC_LS_COMMAND_ICON  "\xef\x86\xb3"

#define PN_LXC_LS_COMMAND_DEFAULT_PERIOD 5u

struct _PnLxcLsCommand
{
...
G_DEFINE_TYPE (PnLxcLsCommand, pn_lxc_ls_command, PN_TYPE_AUTO_TRIGGER)
...
static gchar *
lxc_ls_dup_host_locked (PnLxcLsCommand *self)
{
    gchar *copy;
...
static void
lxc_ls_set_host (PnLxcLsCommand *self, const gchar *host)
{
    gchar *old;
    gchar *replacement;
 ...
 * past the end of @tokens, returns an empty string.  Used to fold any
 * tail tokens of a row past the last header column back into the last
 * cell so a wider-than-expected last column (e.g. a future `lxc-ls`
 * variant whose UNPRIVILEGED slot grows to "true (foo)" or similar)
 * doesn't spawn a phantom extra column. */
static gchar *
...
pn_lxc_ls_command_build_table (const gchar *output)
{
    ...
    if (cleaned->len >= 1)
    {
        ...
        JsonArray   *header_cells;

        for (i = 0; header_words[i] != NULL; i++)
            n_columns++;

        column_ids   = g_new0 (gchar *, n_columns + 1);
        header_cells = json_array_new ();

        for (c = 0; c < n_columns; c++)
        {
            const gchar *text = header_words[c];
            gchar       *id   = sanitise_id (text);
            gchar       *cell_name;
...
static void
pn_lxc_ls_command_trigger (PnAutoTrigger *trigger)
{
    PnLxcLsCommand    *self        = PN_LXC_LS_COMMAND (trigger);
    PnNode            *node        = PN_NODE (self);
    gchar             *host        = lxc_ls_dup_host_locked (self);
    ...
    /* `sudo` invoked verbatim per the node's contract: lxc-ls needs
     * root to enumerate every container.  The host user must have
     * passwordless sudo configured for lxc-ls (no tty is available
     * here, so a password prompt would just fail).  When @host points
     * at a remote machine the same constraint applies *there*, since
     * ssh runs sudo non-interactively too. */
    const gchar       *base_argv[] = { "sudo", "lxc-ls", "-f", NULL };
    gchar            **argv;
    PnSshLogin         login       = { 0 };
...
static void
pn_lxc_ls_command_get_property (
        GObject    *object,
        guint       prop_id,
        GValue     *value,
        GParamSpec *pspec)
{
    PnLxcLsCommand *self = PN_LXC_LS_COMMAND (object);

    switch (prop_id)
    {
    case PROP_HOST:
        g_value_take_string (value, lxc_ls_dup_host_locked (self));
        break;
...
static void
pn_lxc_ls_command_set_property (
        GObject      *object,
        guint         prop_id,
        const GValue *value,
        GParamSpec   *pspec)
{
    ...
    {
    case PROP_HOST:
        lxc_ls_set_host (self, g_value_get_string (value));
        break;
    case PROP_AUTH_PROFILE:
...
static void
on_vault_changed (PnLxcLsCommand *self)
{
    pn_ssh_login_refresh (PN_NODE (self), "auth-profile",
                          &self->mutex, &self->ssh_login);
}
...
static void
pn_lxc_ls_command_finalize (GObject *object)
{
    PnLxcLsCommand *self = PN_LXC_LS_COMMAND (object);
    ...
    G_OBJECT_CLASS (pn_lxc_ls_command_parent_class)->finalize (object);
}

static void
pn_lxc_ls_command_class_init (PnLxcLsCommandClass *klass)
{
    GObjectClass       *object_class  = G_OBJECT_CLASS (klass);
    PnNodeClass        *node_class    = PN_NODE_CLASS (klass);
    PnAutoTriggerClass *trigger_class = PN_AUTO_TRIGGER_CLASS (klass);

    object_class->get_property = pn_lxc_ls_command_get_property;
    object_class->set_property = pn_lxc_ls_command_set_property;
    object_class->finalize     = pn_lxc_ls_command_finalize;
    trigger_class->trigger     = pn_lxc_ls_command_trigger;
...
pn_lxc_ls_command_init (PnLxcLsCommand *self)
{
    ...
    pn_node_set_class_name (node, "Lxc Ls Command");
    pn_node_set_icon       (node, PN_LXC_LS_COMMAND_ICON);
    pn_node_set_color      (node, &blue);
    pn_node_set_has_input  (node, FALSE);
    pn_node_set_has_output (node, TRUE);

    pn_auto_trigger_set_period (PN_AUTO_TRIGGER (self),
                                PN_LXC_LS_COMMAND_DEFAULT_PERIOD);
}
...
PnLxcLsCommand *
pn_lxc_ls_command_new (void)
{
    return g_object_new (PN_TYPE_LXC_LS_COMMAND, NULL);
}
