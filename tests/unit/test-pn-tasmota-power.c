...
/* Unit tests for PnTasmotaPower.  This leaf differs from its siblings
 * only in the meter defaults it pins in _init: it reads the real (active)
 * power off the SENSOR payload at data/payload/ENERGY/Power, scales the
 * dial 0..5000 W, labels it "W" and -- being a derived AC quantity --
 * suppresses the current-type glyph (mode None).  The gate behaviour
 * inherited from PnTasmotaEnergyMeter is covered by the energy-meter
 * test; here we check the leaf's identity and that its own key path
 * resolves and drives the needle, and that it forwards nothing. */
...
test_meter_identity (void)
{
    ...
    PN_CHECK_CMPSTR (key,  ==, "data/payload/ENERGY/Power");
    PN_CHECK_CMPSTR (unit, ==, "W");
...
test_reads_power_channel (void)
{
    ...
    PN_CHECK (needle_revealed (node));         /* ENERGY/Power resolved */
...
{
    pn_test_init (&argc, &argv, "pn-tasmota-power");

    pn_test_add ("meter_identity",      test_meter_identity);
    pn_test_add ("reads_power_channel", test_reads_power_channel);
