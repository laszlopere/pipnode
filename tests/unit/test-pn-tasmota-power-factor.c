...
/* Unit tests for PnTasmotaPowerFactor.  This leaf reads the dimensionless
 * power factor cos(phi) off data/payload/ENERGY/Factor, scales the dial
 * 0..1, labels it with the full "cos φ" (U+03C6) string and -- a quantity
 * describing the V/I phase rather than a current -- suppresses the
 * current-type glyph (mode None).  The gate behaviour inherited from
...
{
    PnNode  *node = g_object_new (PN_TYPE_TASMOTA_POWER_FACTOR, NULL);
    gchar   *key  = NULL;
    gchar   *unit = NULL;
    gdouble  minv = -1.0;
    gdouble  maxv = -1.0;
    ...
    PN_CHECK_CMPSTR (key,  ==, "data/payload/ENERGY/Factor");
    /* "cos φ" -- the φ is the two-byte UTF-8 for U+03C6. */
    PN_CHECK_CMPSTR (unit, ==, "cos \xcf\x86");
    PN_CHECK_NEAR   (minv, 0.0, 1e-9);
    PN_CHECK_NEAR   (maxv, 1.0, 1e-9);
...
static void
test_reads_factor_channel (void)
{
    PnNode    *node = g_object_new (PN_TYPE_TASMOTA_POWER_FACTOR, NULL);
    ...
    PN_CHECK (needle_revealed (node));         /* ENERGY/Factor resolved */
...
{
    pn_test_init (&argc, &argv, "pn-tasmota-power-factor");

    pn_test_add ("meter_identity",       test_meter_identity);
    pn_test_add ("reads_factor_channel", test_reads_factor_channel);
