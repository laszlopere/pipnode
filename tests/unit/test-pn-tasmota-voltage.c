...
/* Unit tests for PnTasmotaVoltage.  This leaf pins an AC voltmeter look
 * in _init: it reads data/payload/ENERGY/Voltage, scales the dial
 * 0..300 V, labels it "V" and paints the AC sine-wave glyph (mode AC).
 * The gate behaviour inherited from PnTasmotaEnergyMeter is covered by
 * the energy-meter test; here we check the leaf's identity and that its
 * own key path resolves and reveals the needle, and that it is a sink. */
...
{
    PnNode  *node = g_object_new (PN_TYPE_TASMOTA_VOLTAGE, NULL);
    gchar   *key  = NULL;
    gchar   *unit = NULL;
    gdouble  minv = -1.0;
    gdouble  maxv = -1.0;
    ...
    PN_CHECK_CMPSTR (key,  ==, "data/payload/ENERGY/Voltage");
    PN_CHECK_CMPSTR (unit, ==, "V");
    PN_CHECK_NEAR   (minv,   0.0, 1e-9);
    PN_CHECK_NEAR   (maxv, 300.0, 1e-9);
...
static void
test_reads_voltage_channel (void)
{
    PnNode    *node = g_object_new (PN_TYPE_TASMOTA_VOLTAGE, NULL);
    ...
    PN_CHECK (needle_revealed (node));         /* ENERGY/Voltage resolved */
...
{
    pn_test_init (&argc, &argv, "pn-tasmota-voltage");

    pn_test_add ("meter_identity",        test_meter_identity);
    pn_test_add ("reads_voltage_channel", test_reads_voltage_channel);
