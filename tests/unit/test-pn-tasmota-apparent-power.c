...
/* Unit tests for PnTasmotaApparentPower.  This leaf reads the apparent
 * power S = V*I off data/payload/ENERGY/ApparentPower, scales the dial
 * 0..5000 (shared with the real/reactive meters), labels it "VA" and --
 * a derived AC-only quantity -- suppresses the current-type glyph (mode
 * None).  The gate behaviour inherited from PnTasmotaEnergyMeter is
 * covered by the energy-meter test; here we check the leaf's identity
 * and that its own key path resolves and reveals the needle, and that it
 * is a sink. */
...
{
    PnNode  *node = g_object_new (PN_TYPE_TASMOTA_APPARENT_POWER, NULL);
    gchar   *key  = NULL;
    gchar   *unit = NULL;
    gdouble  minv = -1.0;
    gdouble  maxv = -1.0;
    ...
    PN_CHECK_CMPSTR (key,  ==, "data/payload/ENERGY/ApparentPower");
    PN_CHECK_CMPSTR (unit, ==, "VA");
...
static void
test_reads_apparent_channel (void)
{
    PnNode    *node = g_object_new (PN_TYPE_TASMOTA_APPARENT_POWER, NULL);
    ...
    PN_CHECK (needle_revealed (node));         /* ENERGY/ApparentPower */
...
{
    pn_test_init (&argc, &argv, "pn-tasmota-apparent-power");

    pn_test_add ("meter_identity",         test_meter_identity);
    pn_test_add ("reads_apparent_channel", test_reads_apparent_channel);
