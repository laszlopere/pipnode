        ...
        GError      **error)
{
    ...
    /* Quote the entire sh -c argument so the remote ssh sees a single
     * shell word.  The script contains single quotes (in `tr -d '|\n'`)
     * so we rely on g_shell_quote to wrap-and-escape correctly rather
     * than hand-rolling the quoting. */
    quoted_script = g_shell_quote (PN_TEMP_REMOTE_SH);
