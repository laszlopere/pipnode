...
/* ------------------------------------------------------------------ */
/*  PnDigitalClock — gui tier.                                         */
/*                                                                     */
/*  The cairo seven-segment LED drawing for the Digital Clock node — the */
/*  Countdown painter with the days group removed, leaving just          */
/*  HOURS:MINUTES:SECONDS on the same red-on-black face with the same     */
/*  segment geometry.  The node's GType, properties, receive() and the    */
/*  hours/minutes/seconds breakdown live in the GTK-free core file        */
/*  pn-digital-clock.c; this file installs the paint_plot vfunc slot onto  */
/*  that class at editor startup (pn_digital_clock_gui_install), reading   */
/*  the drawing state through the core's GTK-free snapshot accessor        */
/*  pn_digital_clock_get_paint_state().  The headless runtime never loads  */
/*  this half, so the Digital Clock logic runs without GTK.                */
/* ------------------------------------------------------------------ */
...
/* Layout ratios, all expressed against the digit cell width D — the same
 * values pn-countdown-gui.c uses, minus the days-group gap. */
#define DCK_H_RATIO       1.70   /* digit height / D                      */
#define DCK_THICK_RATIO   0.18   /* bar thickness / D                     */
#define DCK_INTRA_RATIO   0.14   /* gap between digits in the same group  */
#define DCK_COLON_RATIO   0.50   /* colon column width / D                */
...
static void
pn_digital_clock_paint_plot (PnNode  *node,
                             cairo_t *cr,
                             double   x,
                             double   y,
                             double   w,
                             double   h)
{
    ...
    PnColor                   lit, dim;
    const guint               nd  = 6;   /* HH + MM + SS digit cells */
    double                    pad = 10.0;
    double                    label_h;
    double                    avail_w, digit_top, digit_h;
    double                    D, H, t, s_intra, colon_w, content_w;
    double                    x_cur, digit_y;

    pn_digital_clock_get_paint_state (self, &st);
    ...
    {
        double by_width  = avail_w / ((double) nd * (1.0 + DCK_INTRA_RATIO)
                                      + 2.0 * DCK_COLON_RATIO);
        double by_height = digit_h / DCK_H_RATIO;
    ...
    H       = D * DCK_H_RATIO;
    t       = D * DCK_THICK_RATIO;
    s_intra = D * DCK_INTRA_RATIO;
    colon_w = D * DCK_COLON_RATIO;

    /* content_w sums the six digit cells, the two colon columns and the
     * inter-slot gaps (nd intra gaps). */
    content_w = (double) nd * D + 2.0 * colon_w + (double) nd * s_intra;

    x_cur   = x + (w - content_w) * 0.5;
    digit_y = digit_top + (digit_h - H) * 0.5;

    /* ---- the three digit groups, recording each group's x-range so the
     *      caption can be centred over it. ---- */
    {
        double grp_x0[3], grp_x1[3];
        int    hh[2] = { st.hours   / 10, st.hours   % 10 };
        int    mm[2] = { st.minutes / 10, st.minutes % 10 };
        int    ss[2] = { st.seconds / 10, st.seconds % 10 };
        int    i;

        for (i = 0; i < 3; i++) { grp_x0[i] = 0.0; grp_x1[i] = 0.0; }
        ...
        if (st.show_labels)
        {
            static const gchar *caps[3] = { "HOURS", "MINUTES", "SECONDS" };
...
void
pn_digital_clock_gui_install (void)
{
