...
pn_expression_class_init (PnExpressionClass *klass)
{
    ...
    props[PROP_EXPRESSION] = g_param_spec_string (
            ...
            "1.0 (true) or 0.0 (false). Integer operators: % (modulo), "
            ...
            "log2, log1p; rounding and remainder abs, sign, frac(x) "
            "(the fractional part, always 0..1), and the five verbs "
            "floor, ceil, round (halves away from zero), trunc and "
            "rint (halves to the nearest EVEN), each of which takes an "
            "optional second argument counting decimal places — "
            "round(x, 2) to the cent, round(x, -2) to the hundred; "
            "these QUANTISE a value, they do not format it for display "
            "(that is the Numeric node's decimal places); fmod(x, y), "
