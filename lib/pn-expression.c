...
pn_expression_class_init (PnExpressionClass *klass)
{
    ...
    props[PROP_EXPRESSION] = g_param_spec_string (
            ...
            "copysign(x, y); testing isnan, isinf, isfinite; curves "
            "sinc, erf, erfc, j0, j1; choosing min, max, "
            "clamp(x, lo, hi). An undefined result is a value, not an "
            "error: sqrt(-1) is nan and log(0) is -inf, and both "
            "travel on. Constants: pi and e, which a bound member of "
