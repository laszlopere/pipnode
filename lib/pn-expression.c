...
pn_expression_class_init (PnExpressionClass *klass)
{
    ...
    props[PROP_EXPRESSION] = g_param_spec_string (
            ...
            "asinh, acosh, atanh; powers and logs sqrt, exp, "
            "pow(x, y) (^ is XOR, not a power), hypot(x, y), log(x) "
            "(natural), log(x, base), log10; rounding abs, floor, "
            "ceil, round (halves away from zero), trunc, sign; "
            "choosing min, max, clamp(x, lo, hi). An undefined result "
            "is a value, not an error: sqrt(-1) is nan and log(0) is "
            "-inf, and both travel on. Constants: pi and e, which a "
            "bound member of the same name shadows. "
            "Write several "
