...
pn_expression_class_init (PnExpressionClass *klass)
{
    ...
    props[PROP_EXPRESSION] = g_param_spec_string (
            ...
            "Functions of one argument: sin, cos, tan, asin, acos, atan, "
            "log (natural), log10, exp, sqrt, abs, floor, ceil, round "
            "(halves away from zero), trunc and sign; of two: "
            "atan2(y, x), min, max, pow(x, y) (^ is XOR, not a "
            "power), hypot and log(x, base); of three: "
            "clamp(x, lo, hi). Constants: pi and e, which a bound "
            "member of the same name shadows. "
            "Write several "
