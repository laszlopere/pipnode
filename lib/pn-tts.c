...
/*                                                                     */
/*  Read-only accessors over the file-static engine table so the        */
/*  gui-tier settings dialog can build the engine/voice combos without  */
/*  seeing the table or the private instance struct.                    */
/* ------------------------------------------------------------------ */

guint
pn_tts_n_engines (void)
{
    return (guint) N_ENGINES;
}

const gchar *
pn_tts_engine_id (guint index)
{
    return (index < N_ENGINES) ? engines[index].id : NULL;
}

const gchar *
pn_tts_engine_label (guint index)
{
    return (index < N_ENGINES) ? engines[index].label : NULL;
}

gboolean
pn_tts_engine_installed (guint index)
{
    return (index < N_ENGINES) && engine_is_installed (&engines[index]);
}

const gchar *
pn_tts_engine_label_for_id (const gchar *id)
{
    const TtsEngine *eng = find_engine (id);
    return eng != NULL ? eng->label : NULL;
}

gchar **
pn_tts_engine_list_voices (const gchar *id)
{
    const TtsEngine *eng = find_engine (id);
 ...
 *  message without disturbing the persisted property.  Declared in the
 *  public header so the gui settings dialog's audio preview can call
 *  it; the spawn itself is GTK-free. */
void
pn_tts_speak (
        PnTts       *self,
        const gchar *text,
...
pn_tts_class_init (PnTtsClass *klass)
{
    ...
    /* build_property_editor + build_class_tab installed by the gui tier
     * (pn_tts_gui_install). */
