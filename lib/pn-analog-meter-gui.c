...
/* ------------------------------------------------------------------ */
/*  PnAnalogMeter — gui tier.                                           */
/*                                                                     */
/*  The cairo panel-meter drawing and the settings-dialog tabs for the */
/*  Analog-Meter node.  The node's GType, properties, receive() and the */
/*  damped-spring needle animation live in the GTK-free core file       */
/*  pn-analog-meter.c; this file installs the drawing + dialog vfunc     */
/*  slots onto that class at editor startup (pn_analog_meter_gui_install),*/
/*  reading the drawing state through the core's GTK-free snapshot       */
/*  accessor pn_analog_meter_get_paint_state().  The headless runtime    */
/*  never loads this file's half, so the Analog-Meter logic runs without */
/*  GTK.                                                                */
/* ------------------------------------------------------------------ */
...
/*  Angle helpers                                                      */
/*                                                                     */
/*  Angles are clockwise degrees from 12 o'clock; clock_to_cairo turns */
/*  one into a cairo angle (0 = +x, growing clockwise on screen        */
/*  because y points down), and value_to_angle maps a scale value onto */
/*  the configured start..end sweep using the snapshotted bounds.      */
/* ------------------------------------------------------------------ */
...
static double
value_to_angle (const PnAnalogMeterPaintState *st, double v)
{
    double span = st->max_value - st->min_value;
    double t;

    if (span <= 0.0)
        return st->start_angle;

    if (v < st->min_value) v = st->min_value;
    if (v > st->max_value) v = st->max_value;

    t = (v - st->min_value) / span;
    return st->start_angle + t * (st->end_angle - st->start_angle);
}
...
paint_ticks (cairo_t *cr, const PnAnalogMeterPaintState *st, double radius)
{
    ...
    if (st->major_ticks < 2)
        return;
    if (!(st->max_value > st->min_value))
        return;

    step = pick_nice_step (st->max_value - st->min_value,
                           st->major_ticks);
    if (!(step > 0.0))
        return;

    minors = st->minor_ticks_per_major;
    first  = ceil (st->min_value / step - 1e-9) * step;

    cairo_set_source_rgba (cr,
                           st->scale_color.red,
                           st->scale_color.green,
                           st->scale_color.blue,
                           st->scale_color.alpha);
    ...
    have_prev = FALSE;
    prev_v    = st->min_value;
    for (v = first; v <= st->max_value + step * 1e-9; v += step)
    {
        double seg_lo = have_prev ? prev_v : st->min_value;
        double seg_hi = v;
        ...
        for (j = 1; j < minors; j++)
        {
            ...
            if (mv <= st->min_value || mv >= st->max_value)
                continue;
            ang  = clock_to_cairo (value_to_angle (st, mv));
    ...
    if (have_prev && prev_v < st->max_value)
    {
        double seg_lo = prev_v;
        double seg_hi = st->max_value;
        for (j = 1; j < minors; j++)
        {
            double t   = (double) j / (double) minors;
            double mv  = seg_lo + t * (seg_hi - seg_lo);
    ...
    cairo_set_line_width (cr, 1.8);
    {
        ...
        for (v = first; v <= st->max_value + step * 1e-9; v += step)
        {
            double ang = clock_to_cairo (value_to_angle (st, v));
            cairo_move_to (cr, cos (ang) * r_outer, sin (ang) * r_outer);
            cairo_line_to (cr, cos (ang) * r_major, sin (ang) * r_major);
            if (fabs (v - st->min_value) < step * 1e-6) had_min = TRUE;
            if (fabs (v - st->max_value) < step * 1e-6) had_max = TRUE;
            last_v = v;
        }
        (void) last_v;
        ...
        if (!had_min)
        {
            double ang = clock_to_cairo (value_to_angle (st, st->min_value));
            cairo_move_to (cr, cos (ang) * r_outer, sin (ang) * r_outer);
            cairo_line_to (cr, cos (ang) * r_major, sin (ang) * r_major);
        }
        if (!had_max)
        {
            double ang = clock_to_cairo (value_to_angle (st, st->max_value));
            cairo_move_to (cr, cos (ang) * r_outer, sin (ang) * r_outer);
            cairo_line_to (cr, cos (ang) * r_major, sin (ang) * r_major);
        }
    }
...

    {
        ...
        if (!had_min)
        {
            ...
            format_tick_label (buf, sizeof (buf), st->min_value, step);
            pango_layout_set_text (layout, buf, -1);
            pango_layout_get_pixel_size (layout, &pw, &ph);
            cairo_save (cr);
            cairo_move_to (cr, cx - pw * 0.5, cy - ph * 0.5);
        ...
        if (!had_max)
        {
            ...
            format_tick_label (buf, sizeof (buf), st->max_value, step);
            pango_layout_set_text (layout, buf, -1);
            pango_layout_get_pixel_size (layout, &pw, &ph);
            cairo_save (cr);
            cairo_move_to (cr, cx - pw * 0.5, cy - ph * 0.5);
...
static void
paint_needle (cairo_t *cr, const PnAnalogMeterPaintState *st, double radius)
{
    double                angle;
    ...
    angle = clock_to_cairo (value_to_angle (st, st->display_value));
    {
        ...
        cairo_set_source_rgba (cr,
                               st->needle_color.red,
                               st->needle_color.green,
                               st->needle_color.blue,
                               st->needle_color.alpha);
        cairo_move_to (cr, r0x, r0y);
        cairo_line_to (cr, r1x, r1y);
        cairo_line_to (cr, r2x, r2y);
        cairo_line_to (cr, r3x, r3y);
        ...
        double   h)
{
    ...
    double         lo      = fmin (st.start_angle, st.end_angle);
    double         hi      = fmax (st.start_angle, st.end_angle);
...
void
pn_analog_meter_gui_install (void)
{
